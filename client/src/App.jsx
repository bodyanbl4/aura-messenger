...
// --- КОНФИГУРАЦИЯ ---
// Если сервер запущен локально, используем 127.0.0.1
...
export default function App() {
  ...
  const handleAuth = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        const syncRes = await fetch(`${SERVER_URL}/api/sync?userId=${data.user.id}`);
      ...
      setError("Сервер не отвечает. Проверьте запуск server_index.js");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  if (!user) {
    return (
        <div className="h-screen w-full bg-white flex flex-col items-center justify-center p-8 font-sans">
          ...
          <div className="w-full max-w-[360px] animate-aura-message">
            ...
            <h1 className="text-5xl font-extrabold mb-2 italic tracking-tighter text-center text-black">Aura</h1>
            <p className="text-gray-400 mb-10 text-center font-medium">
              {isRegister ? 'Регистрация нового аккаунта' : 'Добро пожаловать в Aura'}
            </p>
        ...
        <div className={`flex-1 flex flex-col ${activeChatId ? 'hidden' : 'flex'}`}>
          <div className="ios-blur safe-area-top pt-12 pb-2 px-6 sticky top-0 z-20">
            <div className="flex justify-between items-center mb-4">
              <h1 className="text-4xl font-extrabold tracking-tight text-black italic">Aura</h1>
            ...
            {chats.length === 0 ? (
                <div className="text-center mt-32 px-10 animate-aura-message">
                  ...
                  <p className="text-gray-400 font-medium text-center">
                    Список пуст. Зарегистрируйте другого пользователя для теста!
                  </p>
                </div>
            ) : (
                chats.map(chat => (
                    <div key={chat.id} onClick={() => setActiveChatId(chat.id)} className="flex items-center py-4 border-b border-gray-50 ml-6 pr-6 cursor-pointer active:bg-gray-100 transition-colors">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-center mb-1">
                          <h3 className="font-bold text-[17px] truncate text-black">{chat.name}</h3>
          ...
          <div className="safe-area-bottom h-20 border-t border-gray-100 flex justify-around items-center bg-white/95 backdrop-blur-md">
            ...
            <div onClick={() => window.location.reload()} className="flex flex-col items-center text-gray-300 cursor-pointer"><LogOut size={28} strokeWidth={1.5} /><span className="text-[10px] font-medium mt-1">Выйти</span></div>
          </div>
        </div>
        ...
        {activeChatId && activeChat && (
            <div className="fixed inset-0 bg-white z-30 flex flex-col animate-slide-in">
              <div className="ios-blur safe-area-top flex items-center px-4 pt-10 pb-3 border-b border-gray-100">
                ...
                <div className="flex-1 text-center font-bold text-[17px] truncate px-4 text-black">{activeChat.name}</div>
              ...
              <div className="flex-1 overflow-y-auto p-5 space-y-3 no-scrollbar bg-white">
                {activeChat.messages.map((m, i) => {
                  ...
                  return (
                      <div key={i} className={`flex ${isMe ? 'justify-end' : 'justify-start'} animate-aura-message`}>
                        <div className={`max-w-[80%] px-4 py-2.5 rounded-[22px] shadow-sm ${
                            isSticker ? 'sticker-msg text-7xl' : isMe ? 'bg-[#007AFF] text-white rounded-br-none' : 'bg-[#F2F2F7] text-black rounded-bl-none'
                        }`}>
                          {m.text}
                        </div>
                      </div>
              ...
              <div className="safe-area-bottom p-4 border-t flex gap-3 items-center bg-gray-50">
                ...
                <form onSubmit={e => { e.preventDefault(); handleSendMessage(inputText); }} className="flex-1 flex gap-2">
                  <input
                      ...
                      className="flex-1 bg-white border border-gray-200 rounded-full px-4 py-2.5 outline-none font-medium text-black"
